...
### Identifying Applications That Trigger BusyLight (Updated Method)

BusyLight no longer relies on low-level audio session detection (Pycaw), because modern Windows 11 apps (Teams, Chrome, WebView2, etc.) do not reliably expose microphone usage through WASAPI sessions.

Instead, BusyLight uses the Windows Privacy API (CapabilityAccessManager), which tracks real microphone usage at the system level.
